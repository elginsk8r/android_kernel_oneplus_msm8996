...
static void msm_eeprom_imx298_read_vendor_info(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int rc = 0;
...
static ssize_t imx298_eeprom_proc_read(struct file *filp, char __user *buff, size_t len, loff_t *data)
{
	char value[2] = {0};
...
static void msm_eeprom_s5k3p8sp_read_sensor_info(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int rc = 0;
	uint16_t read_data = 0;
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (e_ctrl->userspace_probe == 0) {
		...
		if (strcmp(eb_info->eeprom_name, "sony_imx298") == 0) {
			msm_eeprom_imx298_read_vendor_info(e_ctrl);
			msm_eeprom_proc_init();
		}

		if (strcmp(eb_info->eeprom_name, "s5k3p8sp_m24c64s") == 0) {
			msm_eeprom_s5k3p8sp_read_sensor_info(e_ctrl);
		}
