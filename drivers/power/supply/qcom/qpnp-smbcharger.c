							...
							int current_ma)
{
	...
	pr_smb(PR_STATUS, "USB current_ma = %d, typeC_support_current_ma = %d\n",
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	if (current_limit && current_limit != pre_current_limit) {
		...
		pr_info("change usb_target_current_ma = %d\n",
				current_limit);		rc = vote(chip->usb_icl_votable, PSY_ICL_VOTER, true,
				current_limit);
		if (rc < 0)
...
static void clear_backup_soc(struct smbchg_chip *chip)
{
	...
	rc = smbchg_masked_write(chip, SOC_DATA_REG_0,
		BIT(7) | BIT(6) | BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1),
		(BIT(7) & invalid_soc) | (BIT(6) & invalid_soc) | (BIT(5) & invalid_soc) |
		(BIT(4) & invalid_soc) | (BIT(3) & invalid_soc) | (BIT(2) & invalid_soc) | (BIT(1) & invalid_soc));
	if (rc)
...
static void backup_soc(struct smbchg_chip *chip, int soc)
{
	...
	pr_err("backup_soc[%d]\n", soc);
	soc = (soc << 1) + 1; /* store data in bit1~bit7 */
	rc = smbchg_masked_write(chip, SOC_DATA_REG_0,
		BIT(7) | BIT(6) | BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1),
...
#ifdef CONFIG_PROC_FS
	if (chip->ship_mode) {
		pr_smb(PR_MISC, "smbchg_shutdown enter ship_mode\n");
