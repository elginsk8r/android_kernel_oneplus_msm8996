...
#include <linux/oem_external_fg.h>
							...
							int current_ma)
{
	...
	if (chip->typeC_support_current_ma < current_ma) {
		pr_smb(PR_STATUS, "typeC_support_current_ma = %d\n", chip->typeC_support_current_ma);
...
static void smbchg_re_det_chg_type(struct smbchg_chip  *chip)
{
	...
	if (charger_present != chip->usb_online) {
		...
		prop.intval = chip->usb_online ? 1 : 0;
...
static void smbchg_re_det_work(struct work_struct *work)
{
	...
	/* should add at work actual queueing */
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
	case POWER_SUPPLY_PROP_SDP_CURRENT_MAX:
		if (val->intval <= 2 && get_prop_batt_present(chip)) {
			/* charger removed, set pre_current_ma 0 */
			pre_current_ma = 0;
			chip->chg_ovp = false;
			chip->aicl_current = 0;
		}
...
static irqreturn_t src_detect_handler(int irq, void *_chip)
{
	...
	chip->dash_on = get_prop_fast_chg_started(chip);
	if (chip->dash_on) {
		prop.intval = POWER_SUPPLY_TYPE_DASH;
		power_supply_set_property(chip->usb_psy,
...
static int set_dash_charger_present(int status)
{
	...
	if (g_chip) {
		...
		if (g_chip->dash_present && !pre_dash_present) {
			...
			power_supply_set_property(g_chip->usb_psy,
					POWER_SUPPLY_PROP_TYPE, &prop);
			prop.intval = DEFAULT_WALL_CHG_MA * 1000;
			power_supply_set_property(g_chip->usb_psy,
