...
static void tfa98xx_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	return;
}
 ...
 */
int tfa98xx_get_profile_ctl(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
int tfa98xx_set_profile_ctl(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	 ...
	 * new profile.
	 */
	prof_new->vstep = (int)(tfa98xx->vstep_ctl * prof_new->vsteps / prof_old->vsteps);
	...
	if (tfa98xx_is_amp_running(tfa98xx)) {
		...
		tfa98xx_dsp_start(tfa98xx, tfa98xx->profile_ctl, prof_new->vstep);
	}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	vstep = prof->vsteps - ucontrol->value.integer.value[0] - 1;
	if (vstep == prof->vstep) {
		mutex_unlock(&tfa98xx->dsp_init_lock);
		return 0;
	}
